...
LogInfo("Calling disconnectFromClient due max send recount limit");
...
LogInfo("Calling disconnectFromClient due to execute disconnect");
            DisconnectFromClient(transport_data);
            result = 0;
        }
...
LogInfo("Calling disconnectFromClient Expiry?");
                        DisconnectFromClient(transport_data);
...
LogInfo("Calling disconnectFromClient due to destroy");
        DisconnectFromClient(transport_data);
...
void IoTHubTransport_MQTT_Common_DoWork(TRANSPORT_LL_HANDLE handle)
{
    ...
    if (transport_data != NULL)
    {
        if (InitializeConnection(transport_data) == 0)
        {
            if (transport_data->mqttClientStatus == MQTT_CLIENT_STATUS_PENDING_CLOSE)
            {
                LogInfo("Calling disconnect due to pending close");
...
LogInfo("Calling disconnectFromClient due to keep alive?");
                    DisconnectFromClient(transport_data);
                }
            }
            result = IOTHUB_CLIENT_OK;
